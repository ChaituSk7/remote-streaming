...
/* Play Mp4 file */
int localhost_pipeline(const char *argv) {
	GIOChannel *io_stdin;
	GstBus *bus;
	...
	server_data.audio_resample = gst_element_factory_make("audioresample",
			NULL);
	...
	server_data.rtp_audio_payload = gst_element_factory_make("rtpopuspay",
			NULL);
	...
	gst_bin_add_many(GST_BIN(server_data.pipeline), server_data.source,
			server_data.demuxer, server_data.video_decoder,
			server_data.video_queue, server_data.video_convert,
			server_data.video_encoder, server_data.rtp_payload,
			server_data.udp_sink_video, server_data.audio_decoder,
			server_data.audio_queue, server_data.audio_convert,
			server_data.audio_resample, server_data.audio_volume,
			server_data.audio_encoder, server_data.rtp_audio_payload,
	...
	if (gst_element_link_many(server_data.video_queue,
			server_data.video_decoder, server_data.video_convert,
			server_data.video_encoder, server_data.rtp_payload,
			server_data.udp_sink_video, NULL) != TRUE) {
		g_printerr("Decoder to video udpsink not linked.\n");
		exit (EXIT_FAILURE);
	}

	if (gst_element_link_many(server_data.audio_queue,
			server_data.audio_decoder, server_data.audio_convert,
			server_data.audio_resample, server_data.audio_volume,
			server_data.audio_encoder, server_data.rtp_audio_payload,
	...
	GstPad *sinkpad_audio = gst_element_get_static_pad(
			server_data.udp_sink_audio, "sink");
	GstPad *sinkpad_video = gst_element_get_static_pad(
			server_data.udp_sink_video, "sink");
