...
/* Play Mp3 file */
int hostmp3_pipeline(const char *argv) {
	...
	if (!mp3.pipeline || !mp3.filesrc || !mp3.audio_parse || !mp3.audio_decoder
			...
			|| !mp3.audio_encoder || !mp3.audio_payloader
			|| !mp3.audio_udp_sink) {
	...
	gst_bin_add_many(GST_BIN(mp3.pipeline), mp3.filesrc, mp3.audio_parse,
			mp3.audio_decoder, mp3.audio_queue, mp3.audio_convert,
			mp3.audio_volume, mp3.audio_encoder, mp3.audio_payloader,
			mp3.audio_udp_sink, NULL);

	/* Set the element properties */
	g_object_set(G_OBJECT(mp3.filesrc), "location", argv, NULL);

	g_object_set(G_OBJECT(mp3.audio_udp_sink), "host", "10.1.139.244", "port",
	...
	GstPad *sinkpad_audio = gst_element_get_static_pad(mp3.audio_udp_sink,
			"sink");
